 ...
 * class ExpiringMap, Expirer, delegate, time_to_live,  expiration_interval.
 *
 * In ExpiringMap class, we have a ConcurrentHashMap to save key, value pairs,
 *                       a time_to_live variable for how many millisecond can the ExpiringObject live,
 *                       an expiration_interval, for time to wait until next check, (delay between two check in milliseconds).
 *                       a ExpiringObject for concurrentHashMap value, consists of three records, key, value, lastAccessTime.
 *                       a Expirer object is a java thread, background thread, that's running to check expired objects consistently.
 */
